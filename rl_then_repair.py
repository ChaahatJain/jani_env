...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gurobi_license", help="path to gurobi license" )
...
def repair_metrics(policy: torch.nn.Module, dataset: Any, verbose = False):
        #print(dataset)
        states = torch.tensor(np.array([f["observation"] for f in dataset]), dtype=torch.float32) 
...
def bootstrap_policy_if_needed(args: argparse.Namespace, output_dir: Path) -> Path:
    ...
    if initial is not None and initial.exists():
        logging.info(f"Using existing initial policy: {initial}")
        return initial

    logging.error("No existing policy checkpoint found. Bootstrapping with MaskedPPO training...")
    ...
    logging.info(f"Bootstrapped policy saved at: {bootstrapped}")
    return bootstrapped
...
def main() -> None:
    ...
    if args.repair_method == "milp":
        # --- Gurobi diagnostics ---
        logging.info("=== Gurobi Diagnostics ===")
        logging.info(f"GRB_LICENSE_FILE env var: {os.environ.get('GRB_LICENSE_FILE', 'NOT SET')}")
        home_lic = Path(args.gurobi_license)
        logging.info("==========================")
    ...
    logging.info("Starting fault analysis and repair loop...")
    logging.info(f"Initial policy: {policy_path}")
