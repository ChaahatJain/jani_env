...
def generate_instance(
    ...
    Args:
        ...
        max_speed: Maximum speed in one direction
    ...
    instance_name = f"two_way_line_{num_packages}_{num_locations}"
    ...
    # Generate bounds for the first few and last few locations
    bounds = []
    for i in range(0, max_speed):
        bounds.append({"id": i, "bound": -(i + 1)})
...
def main():
    ...
    parser.add_argument('--speed', '-s', type=int, required=True,
                       help='Maximum speed of the truck')
    ...
    try:
        ...
        output_filename = os.path.join(args.output_dir, f"two_way_line_{args.packages}_{args.locations}.json")
