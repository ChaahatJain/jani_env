...
/* Note: This manual policy is not necessarily perfect for the one way line Icy + Park domain. This is because packages can be nondeterministically dropped during moving. This is highly unlikely and considering this in the policy makes it reach the goal very slowly.*/


// Action ordering for OneWayLine with Parking:
namespace {
    ...
    constexpr int ACTION_PARK = 5;
}


ActionLabel_type OneWayLinePark::get_action(const StateValues& state_values) {
    ...
    const int number_of_locations   = state_values.get_int_state_size() - 6;
    ...
    if (truck_velocity == 0 && packages_here > 0) {
...
// ./PlaJA --engine QL_AGENT --print-stats --evaluation-mode --trace-episodes 50 --save-agent-actions test.json --prop 1 --initial-state-enum sample --num-episodes 3 --max-length-episode 2000 --teacher-name OneWayLine --model-file ../../PlaJABenchmarks/benchmarks_archive/icaps26_fault_fixing/jani_models_and_teachers/one_way_line/models/non_det_no_park/one_way_line.jani --additional-properties ../../PlaJABenchmarks/benchmarks_archive/icaps26_fault_fixing/jani_models_and_teachers/one_way_line/additional_properties/safety_verification/non_det_no_park/compact_starts_no_predicates/one_way_line/pa_one_way_line_compact_starts_no_predicates.jani --ensemble-interface ../../PlaJABenchmarks/benchmarks_archive/icaps26_fault_fixing/jani_models_and_teachers/one_way_line/networks/non_det_no_park/one_way_line/one_way_line_64_64.jani2nnet 